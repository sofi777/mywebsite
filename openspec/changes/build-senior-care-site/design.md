...
**Goals:**
- Static site on Cloudflare Workers free tier
...
- **Cloudflare Workers**: free static hosting via Workers Builds (git auto-deploy); D1 is the natural next step on the same platform for a future database, using the same Worker's own routes rather than a separate product
...
- **Contact method**: embedded Google Form, not `mailto:`/`tel:` links — no personal email/phone needs to be published, and no backend is required since Google Forms handles submission. Uses the user's own form (the same one embedded on their reference Google Sites mockup; submissions go to their email) — confirmed with the user this form is theirs, not the reference business's, before wiring it in.

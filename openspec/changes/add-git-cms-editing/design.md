...
- **CMS tool: Sveltia CMS** (Decap-compatible config format, drop-in replacement) — Decap's simplest auth path (`git-gateway`) requires Netlify Identity, which doesn't exist on Cloudflare; a plain GitHub backend on Decap means self-hosting an OAuth proxy. Sveltia CMS supports GitHub auth via its own hosted OAuth proxy out of the box, so no extra backend/server route is needed on Cloudflare. **Flagged for your confirmation, not locked in** — config-compatible with Decap either way, so switching later is a config change.
...
- **Media**: CMS media library writes to `src/assets/uploads/`, committed to git like everything else. Keeps images in the same git-backed pipeline, no external storage/CDN dependency added.
...
- [Sveltia CMS is smaller/younger than Decap] → drop-in config compatibility means switching back to Decap later is a config change, not a rewrite.
